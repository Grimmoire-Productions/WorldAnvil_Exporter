...
import type { ArticleResponse, ArticleInitialValues } from "~/utils/types";
...
const ArticleProvider: React.FC<{
  ...
  const [activeCharacter, setActiveCharacter] = useState(
    initialValues.activeCharacter,
  );
  const [errorMessage, setErrorMessage] = useState<string | null>(
    initialValues.errorMessage,
  );
  const [articleId, setArticleId] = useState(initialValues.articleId);
  const [isArticleLoading, setIsArticleLoading] = useState(
    initialValues.isArticleLoading,
  );

  const fetchAndProcessCharacter = useCallback(
    async (articleId: string, selectedWorldKey: string) => {
      setIsArticleLoading(true);
      setErrorMessage(null);

      try {
        const results: ArticleResponse =
          await backendAPI.fetchCharacter(articleId);

        if (results?.id) {
          const processedCharacter = await ProcessArticle(
            results,
            selectedWorldKey,
          );
