...
export function processFootnotes(
  ...
  for (const str of arrayContent) {
    ...
    if (hasFootnote) {
      const hasVariableBeforeFootnote =
        hasVariable && str.indexOf("[var:") < str.indexOf("[sup]");
      ...
      */
      ...
      for (const superscript of superscripts) {
        let footnoteIdx = arrayFootnotes.findIndex((note) =>
          note.includes(superscript),
        );
        ...
        const footnoteHasParagraphTags =
          arrayFootnotes[footnoteIdx].includes("<p>") &&
          arrayFootnotes[footnoteIdx].includes("</p>");

        /* Update footnote superscript */
        newString = newString.replace(superscript, `<sup>${footnoteNum}</sup>`);
        arrayFootnotes[footnoteIdx] = arrayFootnotes[footnoteIdx].replace(
          superscript,
          `<sup>${footnoteNum}</sup>`,
        );
        ...
        if (!footnoteHasParagraphTags) {
          arrayFootnotes[footnoteIdx] = "<p>".concat(
            arrayFootnotes[footnoteIdx],
            "</p>",
          );
        }
        footnoteNum += 1;
...
function getVariableName(text: string) {
  return text.substring(
    text.indexOf(":", text.indexOf("var")) + 1,
    text.indexOf("]", text.indexOf("var")),
  );
}

function replaceVariable(
  text: string,
  varData: ReturnType<typeof getVarData>,
  footnoteNum: number,
  ...
  if (varData) {
    return isFirstInstance
      ? text.replace(
          `[var:${varData.name}]`,
          `${varData?.term}<sup>${footnoteNum}</sup>`,
        )
      : text.replace(`[var:${varData.name}]`, varData?.term);
  }
  return text;
}

function addVariableFootnote(
  array: string[],
  footnoteNum: number,
  description: string,
) {
  ...
  */
  const footnoteHeaderIdx = array.findIndex((note) =>
    note.includes(`<h2>Notes</h2>`),
  );

  /* Determine the index of the footnote that will come before this one */
  const footnoteIdx = array.findIndex((note) =>
    note.includes(`<sup>${footnoteNum - 1}</sup>`),
  );
