...
export function correctPunctuation(data: string) {
  ...
  data = data.replaceAll(
    " .",
    ".",
  ); /* Fixes formatting issue caused by prior replaces*/
  data = data.replaceAll(
    " ,",
    ",",
  ); /* Fixes formatting issue caused by prior replaces*/
  ...
  data = data.replaceAll(
    /"(?=(?:(?:[^"]*"){2})*[^"]*"[^"]*$)/g,
    `“`,
  ); /* Replaces opening straight qutoes with curly */
  data = data.replaceAll(
    /"/g,
    `”`,
  ); /* Replaces closing straight quotes with curly */
  data = data.replaceAll(
    /'/g,
    `’`,
  ); /* Replaces straight single quote with curly */
  data = data.replaceAll(
    " ’",
    "’",
  ); /* Fixes formatting issue caused by prior replaces*/
