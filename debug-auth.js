...
async function testWorldAnvilAuth(userToken, appKey) {
  ...
  console.log('User Token:', userToken ? `${userToken.substring(0, 10)}...` : 'null');
...
async function main() {
  ...
  if (!appKey) {
    ...
    console.log('  1. Provide as second argument: node debug-auth.js USER_TOKEN APP_KEY');
  ...
  if (!result1.success) {
    ...
    console.log('4. Ensure both tokens are properly formatted (no extra spaces/newlines)');
  }
}
